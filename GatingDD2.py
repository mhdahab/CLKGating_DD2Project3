...
def clkgating(inputfilename,clockname):
    ...
    for h in inputfile:
        if(h.find("sky130_fd_sc_hd__mux2")!=-1 or h.find("sky130_fd_sc_hd__a21oi")!=-1 ):
            if(h.find("sky130_fd_sc_hd__a21oi")!=-1):
                remove_nor=True
                create_inv+=1
            for g in inputfile:
                if(g.find("[")!=-1):
    ...
    for i in inputfile:
        ...
        if((i.find("module")!=-1)and(i.find("endmodule")==-1)):
            ...
            if(wire_count>0):
                ...
                outputfile.write("  wire "+name+'\n')
            if(create_inv>0):
                outputfile.write("  wire _"+str(create_inv)+"_ ;"+'\n')
                create_inv-=1
